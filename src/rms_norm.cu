...
std::vector<__nv_bfloat16> run_rms_norm(
    ...
    size_t n_elems = static_cast<size_t>(seq_len) * d_model;

    CUDA_CHECK(cudaMalloc(&d_input,  n_elems * sizeof(__nv_bfloat16)));
    CUDA_CHECK(cudaMalloc(&d_output, n_elems * sizeof(__nv_bfloat16)));

    CUDA_CHECK(cudaMemcpy(d_input, h_input, n_elems * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));

    // One block per row; 256 threads is a good default for d_model in [2048, 8192].
    dim3 blockDim(256);
    dim3 gridDim(seq_len);

    rms_norm_kernel<<<gridDim, blockDim>>>(d_input, d_weight, d_output, d_model, eps);
    CUDA_CHECK(cudaDeviceSynchronize());

    std::vector<__nv_bfloat16> h_output(n_elems);
    CUDA_CHECK(cudaMemcpy(h_output.data(), d_output, n_elems * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost));
